...
class GLCMExtractor(Extractor):
                    ...
                    try:
                        if self.orientation_mode == 'separate':
                            ...
                            for prop in self.properties:
                                try:
                                    vals = graycoprops(glcm, prop)[0, :]  # All angles for this distance
                                    features[i + j, feature_idx:feature_idx + len(vals)] = vals
                                    feature_idx += len(vals)
                                except Exception:
                                    # Fill with zeros if computation fails
                                    features[i + j, feature_idx:feature_idx + len(self.angles)] = 0.0
                            ...
                            for prop in self.properties:
                                try:
                                    ...
                                    look_vals = [vals[0], vals[1], (vals[2] + vals[3]) / 2]
                                    features[i + j, feature_idx:feature_idx + 3] = look_vals
                                    feature_idx += 3
                                except Exception:
    ...
    def validate(self):
        ...
        if not isinstance(self.window_sizes, (list, tuple)) or len(self.window_sizes) == 0:
            raise ValueError("window_sizes must be a non-empty list or tuple.")
        for w in self.window_sizes:
            if not isinstance(w, int) or w < 3 or w % 2 == 0:
                raise ValueError(f"Each window size must be an odd integer ≥ 3. Got: {w}")
        if self.levels is not None and self.levels <= 1:
            raise ValueError("levels must be greater than 1 or None for auto-determination.")
        ...
        if self.orientation_mode not in ['separate', 'look_direction', 'average']:
            raise ValueError("orientation_mode must be 'separate', 'look_direction', or 'average'.")